...
static void
print_slice(triton::API &api, Section *sec, uint32_t slice_addr, 
...
main(int argc, char *argv[])
{
  ...
  std::map<triton::arch::registers_e, uint32_t> regs;
...
for(auto &kv: regs) {
  ...
  uint32_t pc         = strtoul(argv[3], NULL, 0);
  uint32_t slice_addr = strtoul(argv[4], NULL, 0);
  Section *sec = bin.get_text_section();

  while(sec->contains(pc)) {
    char mnemonic[32], operands[200], hex_str[10];

    int len = disasm_one(sec, pc, mnemonic, operands);
    if(len <= 0) return 1;
    
    sprintf(hex_str, "0x%x", pc);    
    ...
    for(auto &se: insn.symbolicExpressions) {
      ...
      std::string comment = hex_str; comment += ": "; comment += mnemonic; comment += " "; comment += operands;
    ...
    pc = (uint32_t)api.getConcreteRegisterValue(api.getRegister(ip));
  }
